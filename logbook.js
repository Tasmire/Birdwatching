...
import { StyleSheet, ScrollView, Text, View, ImageBackground, TouchableOpacity, Image, Modal, Dimensions, ActivityIndicator, DeviceEventEmitter } from 'react-native';
...
const Logbook = () => {
        ...
        (async () => {
            try {
                ...
                if (uid) {
                    ...
                    // normalize unlocked info keys so we can check them case-insensitively
    ...
    // subscribe to immediate notifications from mainGame
    useEffect(() => {
        const sub = DeviceEventEmitter.addListener('userInfoUnlocked', (data) => {
            if (!data || !data.animalId || !userId) return;
            // simple strategy: re-fetch unlocked set for this user
            fetchUnlockedForUser(userId, token);
        });
        return () => sub.remove();
    }, [userId, token]);
                        ...
                        <View style={styles.modalOverlay}>
                            <View style={styles.modalContent}>
                                ...
                                <View style={styles.modalDetails}>
                                    {infoFields.map(f => {
                                        const rawProp = f.prop; // e.g. 'maoriName'
                                        ...
                                        // case-insensitive property lookup on modalBird
                                        ...
                                        // If unlocked show actual value (even if empty string), otherwise show placeholder "???"
